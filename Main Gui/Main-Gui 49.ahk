...
;/*
if(IfSettingsActiveOnReload("read")){   ;opens settings window if program was reloaded due to saving the changes
    IniWrite, 0, Config.ini, RunSettingsOnStartup, Run ;IfSettingsActiveOnReload("write") ;resets the config value to 0
    Goto, Settings
}
;*/
return
...
;/*
    ...
    IniRead, WaitDaysUpDown, Config.ini, WaitDaysUpDown, WaitDays
    WaitDaysUpDownOriginalSettings := WaitDaysUpDown

    Gui 2:Add, Edit, w50 x165 y50 hwndUpDownEditHwnd gWhileFocused, %WaitDaysUpDown%
    ;msgbox, %WaitDaysUpDown%
    Gui 2:Add, UpDown, -16 hwndUpDownHwnd vMyUpDown gUpDown Range%RangeMinUpDown%-%RangeMaxUpDown% Wrap, %WaitDaysUpDown%
    Gui 2:Add, button, x-100 y-100 hwndInvisibleButton,Ignore`nthis
    SetUpDown(UpDownHwnd,UpDownEditHwnd,20)
    GuiControlGet, MyUpDown, , %UpDownEditHwnd%   ;Sets value of updown to variable on startup
    WaitDaysUpDown := MyUpDown
    ;================================================= 
    ;*/
...
GuiClose:
	...
	Gui, Cancel
	ExitApp
    IfSettingsActiveOnReload("write")
	}
Return
...
2GuiClose:
    ...
    */
    ...
    GosubIfValidLabel(PopupPromt(%LanguageIni%[19], %LanguageIni%[16],"CloseAndCancelChanges",%LanguageIni%[18] ,"CloseAndSaveChanges", 1,"Cancel","CancelLeavingSettings",0,"Dont show again","",0,HwndMainWindow))
...
CloseAndSaveChanges:
    ...
    ;msgbox, % "TestM: " TestM()
    ;msgbox, hello
    if(ReloadGui(LanguageIni,"CloseAndSaveChangesReloadPopupPromt|ShowPromt",HwndSettingsWindow) = 1){
...
IniFile:
    if(PopupPromt(%LanguageIni%[19], "Cancel","0","Continue","1", 0,"","",1,"I am aware of the possibility of data `n loss caused by manipulation of the `nconfiguration file and wish to continue.","",1,HwndMainWindow,HwndSettingsWindow) = 1){
	    Run, Config.Ini
    }
return
...
IfChangesReversed: 
    ...
    ;MsgBox, temp : %temp%
    ;msgbox, % WaitDaysUpDown "," WaitDaysUpDownOriginalSettings
	if(OnOffOriginalSettings=OnOff && OptionEmptyFoldersOriginalSettings = OptionEmptyFolders && OptionLanguageOriginalSettings=DropDownLanguage && IniPathOriginalSettings=NewPath && IniPath2OriginalSettings=NewPath2 && WaitDaysUpDown = WaitDaysUpDownOriginalSettings)
	{
        ;MsgBox, yes
        GuiControl 2:disable, CancelChanges
        GuiControl 2:disable, SaveChanges
...
UpDown:
    WaitDaysUpDown := MyUpDown    ;updates the final variable
    UnfocusEverything(InvisibleButton)
    if(WaitDaysUpDown!=WaitDaysUpDownOriginalSettings){
        GuiControl 2:enable, CancelChanges
        GuiControl 2:enable, SaveChanges
        ChangedSettings:=1
	}
...
WhileFocused:   ;as long as the contents of updown's edit changes below the time of "IsActive", control stays focused
    ...
    While(IsActive(UpDownEditHwnd,3))     
    {
        ;wait
    }
...
WhileNotFocused:    ;formats the content of updown's edit to valid number and writes it to final variable; calls "WhileActive" once updown's edit is being focused
    ...
    GuiControlGet, MyUpDown, , %UpDownEditHwnd%
    MyUpDown := FormatNumber(MyUpDown)  ;formats edits content

    WaitDaysUpDown := MyUpDown    ;sets content to final variable
    GuiControl,Text, %UpDownEditHwnd%, %MyUpDown%     ;sets formated content to edit

    GuiControlGet, IsFocusedEdit, Focus ;gets focused control's hwnd
    GuiControlGet, IsFocusedHwnd, Hwnd, %IsFocusedEdit%   ;^

    While(IsFocusedHwnd != UpDownEditHwnd)    ;waits until updown's edit is focused
    {
...
UpdateContent:  ;Updates the content of updown's edit after it changed
    ...
    if(CurrentContentEdit < RangeMinUpDown){    ;Checks if value is below min value
        ...
        ;msgbox
        WaitDaysUpDown := CurrentContentEdit  ;updates final value
        GuiControl,Text, %UpDownEditHwnd%, %WaitDaysUpDown%
    }
    ...
    if(IsFocusedHwnd != UpDownEditHwnd){  ;checks if updown's edit is focused and calls correct label accordingly
...
;/*
F7::    ;displays final value trough keystroke
    MsgBox, %WaitDaysUpDown%
return
;*/
...
PopupPromt(GuiText, ButtonBackText, ButtonBackLabel, ButtonContinueText, ButtonContinueLabel, AddButtonCancel,ButtonCancelText,ButtonCancelLabel, AddCheckbox,CheckboxText,CheckboxIniValue, OnlyContinueIfCheckbox, AdditionalOwnershipParam*){
        ...
        if(AddButtonCancel = 1){
           Gui 1:add, Button, gButtonCancel hwndButtonCancelHwnd, %ButtonCancelText% 
        }
    ...
    ButtonCancel:
        ButtonFinalReturn := ButtonCancelLabel2
    return
...
IfSettingsActiveOnReload(IniMode){
    if(IniMode = "read"){
        IniRead, RunSettings, Config.ini, RunSettingsOnStartup, Run, 0
        ;msgbox, RUNSETTIGS: %RunSettings%
        return RunSettings
    }
    else if(IniMode = "write"){
        ;msgbox, %HwndSettingsWindow%
        ;/*
        if(!!WinExist("ahk_id " HwndSettingsWindow) = 1){
            IniWrite, 1, Config.ini, RunSettingsOnStartup, Run
        }
        else if(!!WinExist("ahk_id " HwndSettingsWindow) = 0){
            IniWrite, 0, Config.ini, RunSettingsOnStartup, Run
        }
    
    ;*/
        ;msgbox, % !!WinExist("ahk_id " HwndSettingsWindow)
